...
procinit(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
      ...
      // guard page.
      // 为每个进程分配一个内核栈（一页大小，后面跟着一个无效的守护页，防止栈溢出破坏正常数据）
      char *pa = kalloc();
      if(pa == 0)
        panic("kalloc");
      // 计算当前进程的内核栈的虚拟地址
      uint64 va = KSTACK((int) (p - proc));
      // 内核页表中每个进程内核栈的虚拟地址映射到申请的物理内存上
...
// p->lock must be held.
// 释放proc结构体和进程保持的数据(包括用户页)
static void
freeproc(struct proc *p)
{
  // A kernel page table per process实验开始
  // free the kernel stack in the RAM
  // 注意对于KERNBASE以下的部分，这个process kernel page table只能unmap这些PTE，而不能真的释放这些物理资源.
  // 因为KERNBASE以下的部分其他进程是共享的，因此要先释放掉进程的kstack物理资源，
  // 然后用freewalk_kproc这个函数将pagetable unmap和释放的同时也保留叶节点指向的物理资源
  if (p->kstack)
  {
  ...
  // A kernel page table per process实验结束
...
// but remain leaf physical addresses
void
...
proc_pagetable(struct proc *p)
{
  ...
  // 用户页表映射trampoline代码（supervisor模式下才会使用）
...
// 释放进程的页表并且释放指向的物理内存(TRAMPOLINE、TRAPFRAME例外，因为TRAMPOLINE指向的物理内存是所有进程共享的，TRAPFRAME指向的物理内存会被提前单独释放)
void
